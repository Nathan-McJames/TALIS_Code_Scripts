...
df<-missRanger(df, pmm.k=3, num.trees=20, num.threads=4)
...
X<-as.matrix(one_hot(as.data.table(dfsmall[,-c(6:12, 42, 43, 56, 15, 16)])))
cn<-colnames(X)
...
#Create treatment variable: 1 if teacher has part time contract, 0 otherwise.
W<-ifelse(df$TT3G10A<90, 1, 0)
